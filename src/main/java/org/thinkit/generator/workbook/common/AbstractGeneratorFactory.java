 ...
 * 各業務に必要な生成器が返却されます。
 * <p>
 * 当抽象クラスを継承する全ての具象サブクラスは必ず
 * {@link #createGenerator(GeneratorDivision, DefinitionPath)} を実装する必要があります。
 *
 * @author Kato Shinya
 * @since 1.0
 * @version 1.0
...
abstract class AbstractGeneratorFactory {
     ...
     * {@link #createGenerator(GeneratorDivision, DefinitionPath)}
     * を各業務に対応した生成器を返却するように正しく実装している必要があります。
     * <p>
     * 起動する生成器区分に加えて第2引数として生成器が操作する対象のファイルへのパスを指定する必要があります。<br>
     * 引数として{@code null}が渡された場合は実行時に必ず失敗します。<br>
     * 引数チェックに関しては当メソッドにて実施するため、<br>
     * {@link #createGenerator(GeneratorDivision, DefinitionPath)}
     * の実装時に引数の検査処理を実装する必要はありません。<br>
     * <br>
     * 業務に応じた生成器を取得する際にはサブクラスで実装された
     * {@link #createGenerator(GeneratorDivision, DefinitionPath)} ではなく、<br>
     ...
     * <p>
     * この抽象メソッドは当抽象クラスの {@link #create(GeneratorDivision, DefinitionPath)}
     * で使用されるため、<br>
